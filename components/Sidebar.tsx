...
const NAV_ITEMS = [
  ...
  { id: 'gold-exchange', label: 'Gold Exchange', icon: Repeat },
  
  { id: 'inventory', label: 'Inventory', icon: Package, hidden: true },
  
  // Hidden items (Restricted, not deleted)
  { id: 'layaway', label: 'Layaway', icon: Clock, hidden: true },
  { id: 'advance', label: 'Order Booking', icon: CalendarCheck, hidden: true },
  { id: 'users', label: 'User Management', icon: UserCog, hidden: true },
];
  ...
  return (
    <aside className="w-64 h-screen bg-charcoal-900 border-r border-gray-800 flex flex-col fixed left-0 top-0 z-50 text-white print:hidden">
      <div className="h-16 flex items-center px-6 border-b border-gray-800 gap-3">
         <Logo light />
         <span className="text-sm font-medium tracking-wide text-gray-300">Dhanalakshmi Jewellers</span>
      </div>
